...
class SerialManager():
	...
	def __init__(self, device, baud=115200, waittime = .3,redishost = 'localhost',redisport = 6379,verbose=False,UDPMonitor=False,UDPIp='127.0.0.1',UDPPort=7000, maxconnRetries = math.inf):
		...
		self.maxConnAttempts = maxconnRetries
		self.connRetryAttempt = 0
		self.lastRetryAttempt = 0
			...
			while True:
				try:
					...
					self.__cleanupPacketRecord__()
					 ...
					 #call connect again if COM port disconnected during runtime
					while(self.connRetryAttempt < self.maxConnAttempts): 
						try:
							if ((self.connRetryAttempt - self.lastRetryAttempt) % 4 == 0):  
								self.lastRetryAttempt = self.connRetryAttempt
								print("Retrying connection") #only printing a message every 4 attempts to not spam the cmd line

							time.sleep(5)
							self.connRetryAttempt += 1
							self.__connect__()
							break
